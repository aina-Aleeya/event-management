<section class="bg-white w-full py-16 px-4 sm:px-8 md:px-12 lg:px-20">
    ...
    <div class="grid grid-cols-1 sm:grid-cols-2 lg:grid-cols-3 xl:grid-cols-4 gap-8">
        @forelse ($events as $event)
            <div onclick="window.location='{{ route('ads.click', ['id' => $event->id]) }}'" class="group relative flex flex-col bg-white rounded-xl shadow-md hover:shadow-lg 
                ...
                {{-- Calendar Box --}}
                <div
                    class="absolute top-3 left-3 bg-white/90 backdrop-blur-md rounded-lg text-center shadow-md w-14 border border-gray-200">
                    @if($event->start_date)
                ...
                <div class="flex flex-col flex-grow p-4">
                    ...
                    <div class="flex items-center justify-end mt-auto">
                        <a href="{{ route('ads.click', ['id' => $event->id])  }}" class="text-xs font-semibold uppercase bg-red-700/20 text-black px-3 py-1.5 rounded-md 
                                              hover:bg-red-300 hover:text-gray-900 transition-all duration-150">
                            View Details
                        </a>
                    </div>
                </div>
