<header class="fixed top-0 left-0 z-50 w-full bg-white/95 backdrop-blur-md border-b border-gray-200 shadow-sm">
    ...
    <div class="flex w-full items-center justify-between px-6 lg:px-12 py-4">
        ...
        {{-- Logo --}}
        <a href="{{ route('dashboard') }}" class="flex items-center space-x-3 hover:opacity-90 transition-all group"
            wire:navigate>
            <div
                class="flex w-12 h-12 items-center justify-center rounded-2xl bg-gradient-to-br from-purple-500 via-pink-500 to-red-500 text-white shadow-lg group-hover:shadow-xl group-hover:scale-105 transition-all">
                <x-app-logo-icon class="w-6 h-6 fill-current" />
            </div>
            <span
                class="font-bold text-xl tracking-wide bg-gradient-to-r from-purple-600 to-pink-600 bg-clip-text text-transparent">
                GreatEvent
            </span>
        </a>
            ...
            @auth
                ...
                <div class="relative" x-data="{ userMenuOpen: false }">
                    <button @click="userMenuOpen = !userMenuOpen"
                        ...
                        <span
                            class="inline-flex h-10 w-10 items-center justify-center rounded-xl bg-gradient-to-br from-purple-500 to-pink-500 text-white font-bold shadow-md">
                            {{ auth()->user()->initials() }}
                        </span>
                        <span class="hidden lg:block font-semibold text-gray-700">{{ auth()->user()->name }}</span>
                        <i class="fa-solid fa-chevron-down text-gray-500 text-xs transition-transform"
    ...
    <div x-data="{ mobileMenuOpen: false }" @toggle-mobile-menu.window="mobileMenuOpen = !mobileMenuOpen"
        ...
        x-transition:enter-start="opacity-0 -translate-y-1" x-transition:enter-end="opacity-100 translate-y-0" x-cloak
